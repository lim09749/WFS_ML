...
testpred <- foreach(i=1:folds, .combine=rbind) %dopar%{
  ...
  returnval = matrix(NA, nrow=length(test),ncol=6)
  returnval[,1:6] = cbind(as.character(HAB$Date[test]),
                          as.numeric(as.character(alldata$State[test])),
                          as.numeric(predict(HABsvm,alldata[test,]))-1,
                          as.numeric(predict(HABnbayes,alldata[test,]))-1,
                          predict(HABrvm, alldata[test,]),
                          predict(HABnet, alldata, test))
  returnval
  # anntest = alldata[test,]
  # anntest$State = NULL
  # 
  # returnval = cbind(as.character(HAB$Date[test]),
  #                   as.numeric(as.character(alldata$State[test])),
  #                   svm_prob(HABsvm, alldata[test,]),
  #                   rvm_prob(HABrvm, alldata[test,])[,1],
  #                   ann_prob(HABnet, alldata[test,(1:ncol(alldata)-1)]),
  #                   bay_prob(HABnbayes, alldata[test,]))
}
print(Sys.time()-s)
stopCluster(cl)
colnames(testpred) = c("Date","Real","SVM","NB","RVM","NET")
testpred = as.data.frame(testpred)
write.csv(testpred, 'testpred_k-fold_no_SMOTE.csv')
