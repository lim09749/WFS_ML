...
write.csv(testpred, 'testpred_cv_nutrients.csv')

testpred = read.csv("testpred_cv_nutrients.csv")
testpred$X = NULL
...
plot_ts = function(file_name, width, height, date, abundance, model_type, right){
  ...
  legend(x=date[1]-10, y= 8, 
         legend=c("Observation",
                  ...
                  paste0(model_type," wrong predictions")), 
         col=c("black","green", "red"), 
         lty=c(1,0, 0), lwd=3, pch=c(NA,1, 1),cex=1)
...
# tiff(file = "new SVM CV time series.tiff", width = 18, height = 3.75, units = "in",
#      pointsize=10, res = 300, compression = c("lzw"))
# #par(mfrow=c(1,2))
# plot(HAB$Date, 
...
#        legend=c("Observation",
#                 "SVM correct predictions",
#                 "SVM wrong predictions"), 
#        col=c("black","green"), 
...
# dev.off()
# ####################################################################################
# tiff(file = "new NB CV time series.tiff", width = 7.5, height = 7.5, units = "in",
#      pointsize=10, res = 300, compression = c("lzw"))
# par(mfrow=c(2,1))
