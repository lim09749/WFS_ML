...
filled.contour(x=log10(10^undoscale(xcol, addto1+tn_meanvals[xcol], tn_scalefactors)/10^9/7),
               ...
               plot.axes={axis(side = 1, at = seq(-1.0,1.5,by=0.25),
                               labels=signif(10^seq(-1.0,1.5,by=0.25),digits=2),cex.axis=1.5)
                 axis(side = 2, at = seq(-1.5,1,by=0.25),
                      labels=signif(10^seq(-1.5,1,by=0.25),digits=2),cex.axis=1.5)},
               #axes=F,
               cex.lab=2,
               levels=seq(from=0,to=1,by=0.05),
