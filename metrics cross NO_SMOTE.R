...
#####################################################################################
##Read in data
testpred = read.csv('testpred_k-fold_no_SMOTE.csv')
testpred$X = NULL
...
f1_ = function(x,y){
  2*(recall_(x,y)*precision_(x,y))/(recall_(x,y)+precision_(x,y))
}
...
acc_df[1,] = c(tot_acc(testpred$SVM, testpred$Real),
               ...
               f1_(testpred$SVM, testpred$Real),
...
acc_df[2,] = c(tot_acc(testpred$RVM, testpred$Real),
               ...
               f1_(testpred$RVM, testpred$Real),
...
acc_df[4,] = c(tot_acc(testpred$NET, testpred$Real),
               ...
               f1_(testpred$NET, testpred$Real),
               matthews(testpred$NET, testpred$Real))
#####################################################################################################3
write.csv(acc_df, "cross no smote perf.csv")
